...
class MongoPool(object):
    ...
    def find(self, conditions={}, count=1):
        # 1），实现查询功能，根据条件进行查询，可以指定查询数量，先分数降序，速度升序排，保证优质的代理
        # IP在上面
    ...
    def get_proxies(self,protocol=None,domain=None,count=1,nick_type=0):
        # 2），实现根据协议和要访问的网站的域名，获取代理IP请求；
...
#   测试get_proxies方法
    proxy_list = mongo.get_proxies(count=20,nick_type=2)
    for proxy in proxy_list:
        print(proxy)
    proxy = mongo.random_proxy(nick_type=2)
    print(proxy)
